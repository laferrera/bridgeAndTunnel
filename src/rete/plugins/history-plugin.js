...
function install(editor, { keyboard = true }) {
    ...
    editor.getHistoryJSON = () => {
        // console.log("history.produced", history.produced);
        // console.log("json history.produced", JSON.stringify(history.produced));
        return JSON.parse(JSON.stringify({produced: history.produced, reserved: history.reserved}));
    }

    editor.setHistory = (restoreData) => {
        history.produced =  restoreData.produced;
        history.reserved =  restoreData.reserved;
    }
